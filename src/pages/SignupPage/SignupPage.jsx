...
function SignupPage() {
    ...
    const signupRequest = async (signupData) => {
        try {
            const response = await axios.post("http://localhost:8080/auth/signup", signupData); //1(주소)에다 2(데이터)날릴거다 json은 알아서 axios이 해준다
            // 3000 클라이언트 (프론트 ) 주소
            // 8080 백 주소
            if(response.data) {
        ...
        } catch(error) {//요청에대한 응답이 에러면 catch로 온다
